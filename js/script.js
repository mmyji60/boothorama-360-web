// js/header.js
// Charge /partials/header.html dans #site-header, applique i18n, gère nav et menu déroulant de langue.
// ➕ Propage ?lang=xx dans les liens pour conserver la langue lors de la navigation.

async function loadHeader(options = {}) {
  const {
    mountSelector = '#site-header',
    partialRelPath = 'partials/header.html',
    partialAbsPath = '/partials/header.html',
  ...
  async function fetchFirst(urls) {
    ...
    throw new Error('Aucune des URLs ne répond: ' + urls.join(', '));
  }

  function mountHtml(html) { mount.innerHTML = html; }
  ...
  function wireHeaderBehavior() {
    ...
    links.forEach(a => {
      const file = (a.getAttribute('href') || '').split('/').pop().split('#')[0].split('?')[0];
  ...
  try {
    ...
    if (!html) {
      ...
      html = await fetchFirst([partialRelPath, partialAbsPath]);
      ...
      mount.innerHTML = `
        <div class="container" style="padding:10px 16px">
          <div style="background:#5b1a1a;border:1px solid #9b3b3b;color:#ffdada;padding:10px;border-radius:10px;">
            ⚠️ Impossible de charger <code>${partialRelPath}</code>.<br>
